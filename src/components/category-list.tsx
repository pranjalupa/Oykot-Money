 ...
 * Categories with children render them indented underneath.
 */
...
function Row({
  ...
  const overPlan = !isIncome && cat.actualMinor > cat.plannedMinor;
  const unplanned = cat.plannedMinor === 0 && cat.actualMinor === 0;
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-3">
          <span
            className={cn(
              "truncate text-sm",
              unplanned ? "text-muted-foreground" : "font-medium",
              nested && "text-[13px]",
            )}
          >
            {cat.name}
